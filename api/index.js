...
data_base()
  .then((db) => {
    ...
    const carteira = carteira_module.carteira;
    ...
    console.table(array_visualização);

    array_visualização.map((acao) => {
      if (carteira.includes(acao.ACAO)) {
        console.log("\u001b[1;32m" + acao.ACAO);
      } else {
        console.log("\u001b[1;31m" + acao.ACAO);
      }
    });
  })
  .catch((erro) => {
